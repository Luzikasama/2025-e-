...
uint16_t Add_Corontine(void (*task)(void), uint8_t enable, uint8_t loop,
  ...
  // 使用 calloc 替代 malloc。calloc 会自动将分配的内存全部清零。
  // 这确保了 p->ptr, p->next, p->taskId 等所有成员的初始值都为0。
  scheduler_cron_t *p = (scheduler_cron_t *)calloc(1, sizeof(scheduler_cron_t));
  
  if (p == NULL) {
      return 0xffff; // 内存分配失败
  }
  ...
  p->autodel = autodel;
  // p->ptr 已经被 calloc 初始化为 0，无需手动赋值

  // 将新创建的协程添加到链表末尾
  if (schCronEntry == NULL) {
    // 如果这是第一个协程
    schCronEntry = p;
    // p->taskId 已经被 calloc 初始化为 0，所以第一个任务ID是0
  } else {
    // 如果链表不为空，则遍历到链表末尾
    ...
    q->next = p;
    p->taskId = q->taskId + 1; // 新任务的ID是上一个任务的ID+1
  }
  return p->taskId;
}
